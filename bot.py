...
SERVICES = {
    "tiktok_views": {"name": "TikTok Views", "emoji": "👁️", "api": "tikfames"},
    "tiktok_likes": {"name": "TikTok Likes", "emoji": "❤️", "api": "tikfames"},
    "tiktok_followers": {"name": "TikTok Followers", "emoji": "👥", "api": "tikfames"},
    "tiktok_favorites": {"name": "TikTok Favorites", "emoji": "⭐", "api": "tikfames"},
    "tiktok_shares": {"name": "TikTok Shares", "emoji": "🔗", "api": "tikfames"},
    "instagram_likes": {"name": "Instagram Likes", "emoji": "❤️", "api": "zefame", "service_id": "234"},
    "instagram_views": {"name": "Instagram Views", "emoji": "👁️", "api": "zefame", "service_id": "237"},
    "instagram_followers": {"name": "Instagram Followers", "emoji": "👥", "api": "zefame", "service_id": "233"},
    "facebook_followers": {"name": "Facebook Followers", "emoji": "👥", "api": "zefame", "service_id": "244"},
}
...
def generate_key() -> str:
    ...
    while True:
        key = "BOOST-" + "".join(random.choices(chars, k=12))
        if key not in KEYS:
            return key
...
def redeem_key(user_id: int, key_string: str) -> Dict[str, Any]:
    ...
    key = KEYS[key_string]
    if key.revoked:
        return {"success": False, "message": "❌ Key has been revoked."}
...
def revoke_key(key_string: str) -> Dict[str, Any]:
    ...
    save_keys()
    save_users()
    return {"success": True, "message": f"✅ Key `{key_string}` revoked."}
...
async def get_tiktok_video_details(session: aiohttp.ClientSession, video_url: str, boost_type: str) -> Optional[Dict]:
    ...
    try:
        async with session.post(f'{TIKFAMES_API}/search', headers=get_tikfames_headers(boost_type), json=payload) as response:
        ...
        logger.error(f'TikFames search exception: {e}')
        return None
...
async def process_tikfames_boost(session: aiohttp.ClientSession, video_details: Dict, boost_type: str) -> Dict[str, Any]:
    ...
    try:
        async with session.post(f'{TIKFAMES_API}/process', headers=get_tikfames_headers(boost_type), json=payload) as response:
            text = await response.text()
            if response.status != 200:
                return {'success': False, 'message': f'API error: {response.status}'}
            data = json.loads(text)
            if data.get('success') is True:
                return {'success': True, 'message': data.get('message', 'Boost sent successfully!')}
            return {'success': False, 'message': data.get('message', 'Boost failed')}
    except Exception as e:
        logger.error(f'TikFames process exception: {e}')
...
async def handle_tiktok_boost(session: aiohttp.ClientSession, url: str, service: str) -> Dict[str, Any]:
    ...
    if not video_details:
        return {'success': False, 'message': 'Failed to fetch video details. Please check the URL.'}
...
async def handle_zefame_boost(session: aiohttp.ClientSession, url: str, service: str) -> Dict[str, Any]:
    ...
    try:
        async with session.post(f'{ZEFAME_API}?action=order', headers=headers, data=form_data) as response:
            text = await response.text()
            try:
            ...
            message = result.get('message', 'Order sent successfully!' if success else 'Failed to process order')
...
def key_type_keyboard() -> InlineKeyboardMarkup:
    keyboard = [
        [InlineKeyboardButton("💎 Lifetime", callback_data="keytype:lifetime")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        f"🎉 **Welcome to Premium Boost Bot!** 🎉\n\n"
        f"Hello {user.first_name}! 👋\n\n"
        f"📱 **Status:** {access_status}\n\n"
        f"**Services:**\n"
        f"✅ TikTok Views, Likes, Followers\n"
        f"✅ Instagram Likes, Views, Followers\n"
        f"✅ Facebook Followers\n\n"
    )
    
    if not has_access(user.id) and not is_admin(user.id):
        welcome_text += (
            f"🔑 **Get Access:**\n"
            f"Contact @admin to get your key!\n\n"
            f"💰 **Pricing:**\n"
            f"• 3 Days: $1.99\n"
            f"• 7 Days: $3.99\n"
            f"• 30 Days: $9.99\n"
            f"• Lifetime: $29.99\n\n"
            f"Use '🔑 Get Access' to redeem!"
        )
    else:
        welcome_text += f"🚀 Click 'Boost Now' to start!"
...
async def boost_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if SETTINGS.maintenance_mode and not is_admin(user.id):
        await update.message.reply_text("🔧 Maintenance mode. Try again later.")
        return
    
    if not has_access(user.id):
        await update.message.reply_text(
            "❌ You need access to boost.\n\n"
...
async def handle_service_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("keytype:"):
        ...
        if key_type == "lifetime":
            ...
            await query.edit_message_text(
                f"✅ **Lifetime Key Created!**\n\n"
                f"🔑 Key: `{key.key}`\n"
                f"💎 Duration: Lifetime\n\n"
                f"Send this key to your customer!",
                parse_mode="Markdown"
            )
            return ConversationHandler.END
    ...
    if data.startswith("service:"):
        ...
        await query.edit_message_text(
            f"{service['emoji']} **{service['name']}**\n\n"
            f"Send the URL to boost:\n\n"
            f"Example: https://www.tiktok.com/@user/video/123456\n\n"
...
async def handle_boost_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not service_id or service_id not in SERVICES:
        await update.message.reply_text("❌ Select a service first.")
        return ConversationHandler.END
    ...
    if time.time() - cooldown_time < 30:
        ...
        await update.message.reply_text(f"⏳ Wait {remaining}s before next boost.")
        return ConversationHandler.END
    ...
    if daily_count >= limit:
        await update.message.reply_text(f"❌ Daily limit reached ({limit}/day).")
        return ConversationHandler.END
    ...
    try:
        ...
        await processing_msg.edit_text("❌ Error. Try again later.")
...
async def my_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats_text = (
        ...
        f"🚀 Total Boosts: {user.total_boosts}\n"
...
async def global_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats_text = (
        ...
        f"🔑 Keys: {total_keys}\n"
        f"📍 Used: {used_keys}\n"
        f"🚀 Boosts: {SETTINGS.total_boosts_done}\n"
...
def main():
    ...
    boost_conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex(r'^🚀 Boost Now$'), boost_now),
        ],
        states={
    ...
    async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        elif text == "⚙️ Settings" and is_admin(update.effective_user.id):
